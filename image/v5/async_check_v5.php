...
/** 接口地址 */
define("API_URL", "http://as.dun.163.com/v5/image/asyncCheck");
/** api version */
define("VERSION", "v5");
/** API timeout*/
...
function main(){
	...
	if ($ret["code"] == 200) {
		$result = $ret["result"];
		$checkImages = $ret["checkImages"];
		foreach($checkImages as $index => $checkImage){
		    $name = $checkImage["name"];
		    $taskId = $checkImage["taskId"];
            echo "name={$name}，taskId={$taskId}";
		}
