...
/** 易盾反垃圾云服务图片在线检测接口地址 */
define("API_URL", "http://as.dun.163.com/v5/image/check");
/** api version */
define("VERSION", "v5");
/** API timeout*/
	...
	if ($ret["code"] == 200) {
        ...
        foreach($resultArray as $index => $image_ret){
            ...
            if ($ocr != null) {
                ...
                // 产品需根据自身需求，自行解析处理，本示例只是简单输出结果信息
                foreach($detailArray as $index=>$detail){
                    // 识别ocr文本内容
                    $content = $detail["content"];
                    // ocr片段及坐标信息
                    $lineContents = $detail["lineContents"];
                    echo "识别ocr文本内容:{$content}, ocr片段及坐标信息:".json_encode($lineContents);
                }
            }
            // 图片人脸检测结果
            $face = $ret["face"];
            if ($face != null) {
                ...
                foreach($detailArray as $index=>$detail){
                    ...
                    echo "识别人脸数量:{$faceNumber}, 人物信息及坐标信息:".json_encode($faceContents);
                }
            }
            ...
            if ($quality != null) {
                ...
                foreach($detailArray as $index=>$detail){
                    ...
                    // 图片清晰度分数
                    $sharpnessRate = $detail["sharpnessRate"];
                    // 图片基本信息
                    $metaInfo = $detail["metaInfo"];
                    // 图片边框信息
                    $boarderInfo = $detail["boarderInfo"];
                    // 图片背景信息
                    $backgroundInfo = $detail["backgroundInfo"];
                    echo "quality detail:".json_encode($detail);
                }
            }
            ...
            $discern = $ret["discern"];
            if ($discern != null) {
                $detailArray = $discern["details"];
                // 产品需根据自身需求，自行解析处理，本示例只是简单输出结果信息
                foreach($detailArray as $index=>$detail){
                    $type = $detail["type"];
                    $discernName = $detail["discernName"];
                    $rate = $detail["rate"];
                    echo "discern detail:".json_encode($detail);
                }
            }

            // 用户画像结果
            $userRisk = $ret["userRisk"];
            if ($userRisk != null) {
                $detailArray = $userRisk["details"];
                // 产品需根据自身需求，自行解析处理，本示例只是简单输出结果信息
                foreach($detailArray as $index=>$detail){
                    $account = $detail["account"];
                    $accountLevel = $detail["accountLevel"];
                    echo "userRisk detail:".json_encode($detail);
                }
            }
